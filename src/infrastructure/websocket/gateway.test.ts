...
describe('WebSocketGateway', () => {
  ...
  it('subscribes event handlers and emits to route contracts', () => {
    ...
    const eventBus = {
      subscribe: jest.fn((eventName: string, listener: (event: any) => void) => {
    ...
    listeners.get(MessageSentEvent.name)?.(messageEvent);
    ...
    listeners.get(UserTypingEvent.name)?.(typingEvent);
    ...
    listeners.get(NotificationSentEvent.name)?.(notificationEvent);
